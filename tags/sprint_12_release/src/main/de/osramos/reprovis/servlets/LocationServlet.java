	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		if(id == null) {
			...
			if(req.getParameter("getData") != null) {
				...
				req.setAttribute("location", GlobalBean.getInstance().getRegistry().lookup(Integer.valueOf(id)));
				getServletContext().getRequestDispatcher("/LocationJSON.jsp").forward(req, resp);
			}
			
			else {
				// Call the view handler
				req.setAttribute("locid", GlobalBean.getInstance().getRegistry().lookup(Integer.valueOf(id)));
